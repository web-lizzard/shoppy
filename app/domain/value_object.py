...
class Money:
    ...
    @classmethod
    def mint(cls, amount: float | Decimal, currency: Currency = Currency.PLN) -> 'Money':
    ...
    def __add__(self, other: 'Money') -> 'Money':
        if self.currency != other.currency:
            raise ValueError("Adding two money representation with different currency is not allowed!")
        return self.__class__(amount=self.amount + other.amount, currency=self.currency)
    
    def __sub__(self, other: 'Money') -> 'Money':
        if self.currency != other.currency:
            raise ValueError("Subtracting two money representation with different currency is not allowed!")
        return self.__class__(amount=self.amount - other.amount, currency=self.currency)
